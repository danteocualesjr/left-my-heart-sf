"use client";

import { Story } from "@/lib/types";
import NeighborhoodBadge from "./NeighborhoodBadge";
import SocialLinks from "./SocialLinks";
import Link from "next/link";

interface StoryCardProps {
  story: Story;
  compact?: boolean;
}

export default function StoryCard({ story, compact = false }: StoryCardProps) {
  return (
    <div
      className={`bg-cream rounded-3xl shadow-xl overflow-hidden flex flex-col ${
        compact ? "max-w-sm" : "w-full max-w-md"
      }`}
    >
      {/* Top accent bar */}
      <div className="h-1.5 bg-gradient-to-r from-gate-red via-sunset to-bay" />

      <div className={`flex flex-col flex-1 ${compact ? "p-5" : "p-6 sm:p-8"}`}>
        {/* Header */}
        <div className="flex items-start justify-between gap-3">
          <NeighborhoodBadge name={story.neighborhood} />
          <span className="text-xs text-ink-light font-body whitespace-nowrap">
            {story.date}
          </span>
        </div>

        {/* Title */}
        <h3
          className={`font-display font-bold text-fog-dark mt-4 ${
            compact ? "text-lg" : "text-xl sm:text-2xl"
          }`}
        >
          {story.title}
        </h3>

        {/* Body */}
        <div
          className={`mt-3 font-body text-ink-light leading-relaxed flex-1 overflow-y-auto ${
            compact
              ? "text-sm max-h-40 line-clamp-6"
              : "text-sm sm:text-base max-h-64 sm:max-h-80"
          }`}
        >
          {story.body.split("\n").map((paragraph, i) => (
            <p key={i} className={i > 0 ? "mt-3" : ""}>
              {paragraph}
            </p>
          ))}
        </div>

        {/* Footer */}
        <div className="mt-5 pt-4 border-t border-cream-dark">
          <div className="flex items-center justify-between">
            <div>
              <Link
                href={`/profile/${story.authorId}`}
                className="font-body text-sm font-semibold text-fog-dark hover:text-gate-red transition-colors"
              >
                {story.authorName}
              </Link>
              <div className="flex items-center gap-1 mt-0.5">
                <svg
                  width="14"
                  height="14"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  className="text-gate-red"
                >
                  <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                </svg>
                <span className="text-xs text-ink-light font-body">
                  {story.hearts}
                </span>
              </div>
            </div>
            <SocialLinks links={story.socialLinks} />
          </div>
        </div>
      </div>
    </div>
  );
}
